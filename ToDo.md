...
- [ ] Gerüst-Editor mit Struktur-Zwang: Manuelles Bearbeiten von geruest.md (z.B. nach "Neu schreiben") kann das Gerüst unbemerkt strukturell ungültig machen, obwohl es inhaltlich vollständig aussieht - z.B. wenn beim Editieren des Kapitelplans versehentlich alle "Zielwortzahl: X–Y Wörter"-Zeilen entfernt werden. Die Kapitel-Überschriften bleiben dann zwar sichtbar, aber `kapitelplan_erkennen()` (backend/app/core/geruest.py) erkennt OHNE die Wörter-Angabe pro Kapitel gar keine Kapitel mehr, `letztes_geplantes_kapitel()` liefert `None`, und der Automatik-Schreiber meldet fälschlich "keine Kapitel-Struktur" (Vorfall: a-Blut-und-Ahornlaub-Die-Ehre-des-Verbotenen, 2026-08-20). Statt freiem Markdown-Textfeld einen strukturierten Editor bauen (eigene Formularfelder pro Kapitel: Titel, Ort, Figuren, Ereignis, Zielwortzahl als Pflichtfeld etc. statt Freitext-Bullet-Liste), der es technisch unmöglich macht, ein Pflichtfeld wie die Zielwortzahl zu vergessen. Mindestens aber: vor dem Speichern eines manuell editierten Gerüsts serverseitig validieren (`kapitelplan_erkennen()` muss für jede im Kapitelplan-Abschnitt vorkommende "Kapitel N"-Überschrift auch einen Eintrag liefern) und bei fehlender Zielwortzahl mit klarer Fehlermeldung ablehnen, statt es klaglos zu speichern.
# Bugs
